...
dataset = {
    "data":{"filename":"/sps/km3net/users/alflazo/dstProd/v7.1_v7.2_jsh/mcv7.1.gsg_anti-elec-CC_100-500GeV.sirene.jorcarec.jsh.aanet.dst_merged.root"}
}
...
hists = [
    ...
    ka.histogram("energy_vs_cos_theta",
                 ["E.trks.E[:,0]","E.trks.dir.z[:,0]"],
                 [10**np.linspace(0,4,41),np.linspace(-1,1,21)])
]

manager.append_module("my_simple_hists_without_cuts",
                      ka.histogramHandler, 
                      kwargs = {'histograms':hists})

manager.append_module("my_simple_hists_with_cuts",
                      ka.histogramHandler, 
