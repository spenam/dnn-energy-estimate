...
with h5py.File("shuffled_dsts.h5", "r") as f:
    full_data = f["data"]
    if lkl== 0:
        data_scaled = tf.convert_to_tensor(pd.DataFrame(full_data['JSHOWERFIT_LENGTH_METRES', 'JSHOWERFIT_ENERGY', 'JENERGY_ENERGY', 'trig_hits', 'trig_doms', 'trig_lines', 'JSTART_LENGTH_METRES', 'dir_x_gandalf', 'dir_y_gandalf', 'dir_z_gandalf','dir_x_showerfit', 'dir_y_showerfit', 'dir_z_showerfit']).to_numpy())
    elif lkl == 5:
        data_scaled = tf.convert_to_tensor(pd.DataFrame(full_data['JSHOWERFIT_LENGTH_METRES', 'JSHOWERFIT_ENERGY',
    ...
    with h5py.File(
        ...
        "pred_y/" + NN_info + "_w_allDSTs_" + str(n_features) + "_" + str(LR) + ".h5", "w"
    ) as g:
...
with h5py.File("for_pred.h5", "r") as f:
    ...
    with h5py.File(
        ...
        "pred_y/" + NN_info + "_w_blindDSTs_" + str(n_features) + "_" + str(LR) + ".h5", "w"
    ) as g:
        #g.create_dataset("y", data=y_pred)
        g["y"]=np.log10(f["data"]["mc_energy_dst"])
        g["JENERGY"]=f["data"]['JENERGY_ENERGY']
