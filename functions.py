...
def train_predict(
    ...
    print(
        "training {} model, sample size {}.".format(
...
def hist_sampling(uplim=60000, lowE=0, highE=2, nbins=20, mcE=0):
    ...
    for i in range(len(Ebins) - 1):
        ...
        if len(y_all_temp) > uplim:
            ...
            print("###### THIS IS JOINT MASK #######",
                  "n true: ", joint_mask.sum())
...
def bootstrap_sampling(data, uplim=400, lowE=0, highE=2, nbins=20):
    Ebins = np.logspace(lowE, highE, nbins)
    ran_state = 0
    histo = np.histogram(data["mc_energy_dst"],
                         bins=Ebins, weights=data["weights"])
    cdata = data.copy()
    for i in range(len(Ebins) - 1):
        print(
            "Doing for bin "
        ...
        mask = (data["mc_energy_dst"] > Ebins[i]) & (
            data["mc_energy_dst"] < Ebins[i + 1]
        )
        data_temp = data[mask]
        if i == 0:
            frac_use = (
                (uplim) / np.mean(data_temp["weights"]
                                  ) / len(data_temp["weights"])
            )
            resampled_df = data_temp.sample(
                frac=frac_use, random_state=ran_state, replace=True
            )
        else:
            frac_use = (
                (uplim) / np.mean(data_temp["weights"]
