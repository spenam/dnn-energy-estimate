...
def do_plots(fname, plot_path):
    ...
    if not os.path.exists(plot_path + "/performance"):
        os.makedirs(plot_path + "/performance")
    fig, ax = plt.subplot_mosaic(
            ...
            IIJJ
            """,
            figsize=(xsize*xnum, ysize*ynum), constrained_layout=True
    )
    ...
    for i in range(xnum):
        ...
        do_2dcorr(ax[ax0[i]], y, recos[i], histweights=weights, x_label=ltrueE, y_label=reco_labels[i])
        do_2dcorr(ax[ax1[i]], y, recos[i]*gfg(recos[i]),histweights=weights, y_label=reco_unbiased_labels[i], x_label=ltrueE)
        #plot_unbiasing_factor(ax[ax3[i]], y, recos[i], histweights=weights)
        plot_unbiasing_factor_numu_nue(ax[ax3[i]], y, recos[i], histweights=weights)
    ...
    fig.suptitle(tit)
    plt.savefig(plot_path +'/performance/'+ tit+'.png')
    plt.savefig(plot_path +'/performance/'+ tit+'.pdf')
    plt.clf()
    exit()
#plt.show()
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-n", "--Name", help="Name of h5 file", required=True
    )
