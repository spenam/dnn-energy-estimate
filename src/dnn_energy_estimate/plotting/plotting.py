...
def avg_and_std(h, x, y, err = 0):
    ...
    """
    n, _ = h.shape
    out = np.zeros((n, 3)) # 3 columns with: avg xbin, weighted_avg, and weighted_std
...
def normal_2d_compare(X_test0, y_test0, y_pred0, mycmap = "BuPu", name_suffix = "draft", Gcols = 0, Jshf = 0, n_features = 0, LR = 0.1, wstr = "w", save = True ):
    cmin = 1e-10
    ...
    fig, ax = plt.subplots(dims[0], dims[1],  figsize = (6*dims[1], 4*dims[0]),constrained_layout=True )
    nbines = 50
    ...
    im = ax[0][0].hist2d(10**y_test0,10**y_pred0, bins=[x_bins, y_bins], range =[xlim,ylim], weights =
                         X_test0['weights'], cmap=mycmap, cmin = cmin)
    avg = avg_and_std(im[0],im[1],im[2])
    avg_x, avg_y, std = avg[:, 0], avg[:, 1], avg[:, 2]
    ax[0][col].errorbar(avg_x, avg_y, yerr=std, fmt='.', markersize=6, linewidth=1, c="black", label='weighted mean and std')
    ax[0][col].plot(10**y_test0,10**y_test0,color="black")
    ax[0][col].set_xlabel(r"$E_{True}$ [GeV]")
    ax[0][col].set_ylabel(r"$E_{Est}$ [GeV]")
    with open("plots/"+name_suffix+"/"+wstr+"/"+str(LR)+"/Eest.pkl","wb") as f:
        pkl.dump(10**y_test0, f)
        pkl.dump(X_test0['weights'], f)
    with open("plots/"+name_suffix+"/"+wstr+"/"+str(LR)+"/ETrue.pkl","wb") as f:
    ...
    im = ax[0][col].hist2d(10**y_test0,10**X_test0['JENERGY_ENERGY'], bins=[x_bins, y_bins], range =[xlim,ylim], weights = X_test0['weights'], cmap=mycmap, cmin = cmin)
    avg = avg_and_std(im[0],im[1],im[2])
    avg_x, avg_y, std = avg[:, 0], avg[:, 1], avg[:, 2]
    ax[0][col].errorbar(avg_x, avg_y, yerr=std, fmt='.', markersize=6, linewidth=1, c="black", label='weighted mean and std')
    ax[0][col].plot(10**y_test0,10**y_test0,color="black")
    ...
    with open("plots/"+name_suffix+"/"+wstr+"/"+str(LR)+"/JENERGY.pkl","wb") as f:
    ...
    im = ax[0][col].hist2d(10**y_test0,10**(X_test0['JSTART_LENGTH_METRES'])*0.25, bins=[x_bins, y_bins], range =[xlim,ylim],
                         weights = X_test0['weights'], cmap=mycmap, cmin = cmin)
    avg = avg_and_std(im[0],im[1],im[2])
    avg_x, avg_y, std = avg[:, 0], avg[:, 1], avg[:, 2]
    ax[0][col].errorbar(avg_x, avg_y, yerr=std, fmt='.', markersize=6, linewidth=1, c="black", label='weighted mean and std')
    ax[0][col].plot(10**y_test0,10**y_test0,color="black")
    ...
    with open("plots/"+name_suffix+"/"+wstr+"/"+str(LR)+"/JSTART.pkl","wb") as f:
    ...
    if Jshf != 0:
        ...
        y_bins = np.logspace(ylim_log[0], ylim_log[1], nbines)
        #im = ax[0][col].hist2d(10**y_test0,X_test0['JENERGY_ENERGY'], bins=[x_bins, y_bins], norm=mpl.colors.LogNorm(), range =[xlim,ylim])
        im = ax[0][col].hist2d(10**y_test0,10**X_test0['JSHOWERFIT_ENERGY'], bins=[x_bins, y_bins], range =[xlim,ylim], weights = X_test0['weights'], cmap=mycmap, cmin = cmin)
        avg = avg_and_std(im[0],im[1],im[2])
        avg_x, avg_y, std = avg[:, 0], avg[:, 1], avg[:, 2]
        ax[0][col].errorbar(avg_x, avg_y, yerr=std, fmt='.', markersize=6, linewidth=1, c="black", label='weighted mean and std')
        ax[0][col].plot(10**y_test0,10**y_test0,color="black")
        ...
        with open("plots/"+name_suffix+"/"+wstr+"/"+str(LR)+"/JSHOWERFIT.pkl","wb") as f:
    ...
    if len(Gcols)>0:
        ...
        im = ax[0][col].hist2d(10**y_test0,10**X_test0['pred_energy'], bins=[x_bins, y_bins], range =[xlim,ylim], weights =
                             X_test0['weights'], cmap=mycmap, cmin = cmin)
        avg = avg_and_std(im[0],im[1],im[2])
        avg_x, avg_y, std = avg[:, 0], avg[:, 1], avg[:, 2]
        ax[0][col].errorbar(avg_x, avg_y, yerr=std, fmt='.', markersize=6, linewidth=1, c="black", label='weighted mean and std')
        ...
        with open("plots/"+name_suffix+"/"+wstr+"/"+str(LR)+"/GNN.pkl","wb") as f:
...
def inv_2d_compare(X_test0, y_test0, y_pred0, mycmap = "BuPu", name_suffix = "draft", Gcols = 0, Jshf = 0, n_features = 0, LR= 0.1, wstr = "w", save = True):
    cmin = 1e-10
    n_features = str(n_features)
    if len(Gcols)>0:
        if Jshf != 0:
    ...
    im = ax[0][col].hist2d(10**y_pred0, 10**y_test0, bins=[x_bins, y_bins], range =[xlim,ylim], weights =
                         X_test0["weights"], cmap=mycmap, cmin = cmin)
    avg = avg_and_std(im[0],im[1],im[2])
    avg_x, avg_y, std = avg[:, 0], avg[:, 1], avg[:, 2]
    ax[0][col].errorbar(avg_x, avg_y, yerr=std, fmt='.', markersize=6, linewidth=1, c="black", label='weighted mean and std')
    ...
    im = ax[0][col].hist2d( 10**X_test0['JENERGY_ENERGY'], 10**y_test0, bins=[x_bins, y_bins], range =[xlim,ylim], weights = X_test0["weights"], cmap=mycmap, cmin = cmin)
    avg = avg_and_std(im[0],im[1],im[2])
    avg_x, avg_y, std = avg[:, 0], avg[:, 1], avg[:, 2]
    ax[0][col].errorbar(avg_x, avg_y, yerr=std, fmt='.', markersize=6, linewidth=1, c="black", label='weighted mean and std')
    ax[0][col].plot(10**y_test0,10**y_test0,color="black")
    ...
    im = ax[0][col].hist2d(10**(X_test0['JSTART_LENGTH_METRES'])*0.25, 10**y_test0, bins=[x_bins, y_bins], range =[xlim,ylim], weights = X_test0["weights"], cmap=mycmap, cmin = cmin)
    avg = avg_and_std(im[0],im[1],im[2])
    avg_x, avg_y, std = avg[:, 0], avg[:, 1], avg[:, 2]
    ax[0][col].errorbar(avg_x, avg_y, yerr=std, fmt='.', markersize=6, linewidth=1, c="black", label='weighted mean and std')
    ax[0][col].plot(10**y_test0,10**y_test0,color="black")
    ...
    y_bins = np.logspace(ylim_log[0], ylim_log[1], nbines)
    #im = ax[0][col].hist2d( X_test0['JENERGY_ENERGY'], 10**y_test0, bins=[x_bins, y_bins], norm=mpl.colors.LogNorm(), range =[xlim,ylim])
    im = ax[0][col].hist2d( 10**X_test0['JSHOWERFIT_ENERGY'], 10**y_test0, bins=[x_bins, y_bins], range =[xlim,ylim], weights = X_test0["weights"], cmap=mycmap, cmin = cmin)
    avg = avg_and_std(im[0],im[1],im[2])
    avg_x, avg_y, std = avg[:, 0], avg[:, 1], avg[:, 2]
    ax[0][col].errorbar(avg_x, avg_y, yerr=std, fmt='.', markersize=6, linewidth=1, c="black", label='weighted mean and std')
    ax[0][col].plot(10**y_test0,10**y_test0,color="black")
    ...
    if len(Gcols)>0:
        ...
        im = ax[0][col].hist2d( 10**X_test0['pred_energy'], 10**y_test0, bins=[x_bins, y_bins], range =[xlim,ylim], weights = X_test0["weights"], cmap=mycmap, cmin = cmin)
        avg = avg_and_std(im[0],im[1],im[2])
        avg_x, avg_y, std = avg[:, 0], avg[:, 1], avg[:, 2]
        ax[0][col].errorbar(avg_x, avg_y, yerr=std, fmt='.', markersize=6, linewidth=1, c="black", label='weighted mean and std')
        ax[0][col].plot(10**y_test0,10**y_test0,color="black")
...
def error_plots(X_test0, y_test0, y_pred0, mycmap = "viridis", name_suffix = "draft", Gcols = 0, n_features = 0, LR =
                0.1, wstr = "w", save = True):
    cmin = 1e-10
    n_features = str(n_features)
    ...
    im = ax[0][0].hist2d(10**y_test0,((10**y_pred0-10**y_test0)/10**y_test0), bins = [x_bins, y_bins],
                         range =[xlim,error_ylim], weights = X_test0['weights'], cmin = cmin)
    avg = avg_and_std(im[0],im[1],im[2], err=1)
    avg_x, avg_y, std = avg[:, 0], avg[:, 1], avg[:, 2]
    ax[0][0].errorbar(avg_x, avg_y, yerr=std, fmt='.', markersize=6, linewidth=1, c="black", label='weighted mean and std')
    ax[0][0].plot(10**y_test0,np.ones(len(y_test0)),color="red",linestyle="--")
    ...
    im = ax[1][0].hist2d(10**y_test0,((y_pred0-y_test0)/y_test0), bins = [x_bins, y_bins], range =[xlim,[-2.5,2.5]], weights = X_test0['weights'], cmin = cmin)
    avg = avg_and_std(im[0],im[1],im[2], err=1)
    avg_x, avg_y, std = avg[:, 0], avg[:, 1], avg[:, 2]
    ax[1][0].errorbar(avg_x, avg_y, yerr=std, fmt='.', markersize=6, linewidth=1, c="black", label='weighted mean and std')
    ax[1][0].plot(10**y_test0,np.ones(len(y_test0)),color="red",linestyle="--")
    ax[1][0].plot(10**y_test0,np.zeros(len(y_test0)),color="red")
    ax[1][0].plot(10**y_test0,-np.ones(len(y_test0)),color="red",linestyle="--")
    ax[1][0].set_ylabel(r"$\frac{L_{10}E_{Est}-L_{10}E_{True}}{L_{10}E_{True}}$")
    ...
    im = ax[2][0].hist2d(10**y_test0,np.abs((10**y_pred0-10**y_test0)/10**y_test0), bins = [x_bins, y_bins], range
                         =[xlim,[0, error_ylim[1]]], weights = X_test0['weights'], cmin = cmin)
    avg = avg_and_std(im[0],im[1],im[2], err=1)
    avg_x, avg_y, std = avg[:, 0], avg[:, 1], avg[:, 2]
    ax[2][0].errorbar(avg_x, avg_y, yerr=std, fmt='.', markersize=6, linewidth=1, c="black", label='weighted mean and std')
    ax[2][0].plot(10**y_test0,np.ones(len(y_test0)),color="red",linestyle="--")
    ax[2][0].plot(10**y_test0,0.5*np.ones(len(y_test0)),color="red",linestyle="--")
    ax[2][0].plot(10**y_test0,0.25*np.ones(len(y_test0)),color="red",linestyle="--")
    ...
    im = ax[3][0].hist2d(10**y_test0,np.abs((y_pred0-y_test0)/y_test0), bins = [x_bins, y_bins], range =[xlim,[0,2.5]],
                        weights = X_test0['weights'], cmin = cmin)
    avg = avg_and_std(im[0],im[1],im[2], err=1)
    avg_x, avg_y, std = avg[:, 0], avg[:, 1], avg[:, 2]
    ax[3][0].errorbar(avg_x, avg_y, yerr=std, fmt='.', markersize=6, linewidth=1, c="black", label='weighted mean and std')
    ax[3][0].plot(10**y_test0,np.ones(len(y_test0)),color="red",linestyle="--")
    ax[3][0].plot(10**y_test0,0.5*np.ones(len(y_test0)),color="red",linestyle="--")
    ax[3][0].plot(10**y_test0,0.25*np.ones(len(y_test0)),color="red",linestyle="--")
    ax[3][0].set_ylabel(r"$\frac{|L_{10}E_{Est}-L_{10}E_{True}|}{L_{10}E_{True}}$")
    ...
    im = ax[0][1].hist2d(10**y_test0,((10**X_test0['JENERGY_ENERGY']-10**y_test0)/10**y_test0), bins = [x_bins, y_bins],
                         range =[xlim,error_ylim], weights = X_test0['weights'], cmin = cmin)
    avg = avg_and_std(im[0],im[1],im[2], err=1)
    avg_x, avg_y, std = avg[:, 0], avg[:, 1], avg[:, 2]
    ax[0][1].errorbar(avg_x, avg_y, yerr=std, fmt='.', markersize=6, linewidth=1, c="black", label='weighted mean and std')
    ax[0][1].plot(10**y_test0,np.ones(len(y_test0)),color="red",linestyle="--")
    ...
    im = ax[1][1].hist2d(10**y_test0,((X_test0['JENERGY_ENERGY']-y_test0)/y_test0), bins = [x_bins, y_bins],
                         range =[xlim,[-2.5,2.5]], weights = X_test0['weights'])
    avg = avg_and_std(im[0],im[1],im[2], err=1)
    avg_x, avg_y, std = avg[:, 0], avg[:, 1], avg[:, 2]
    ax[1][1].errorbar(avg_x, avg_y, yerr=std, fmt='.', markersize=6, linewidth=1, c="black", label='weighted mean and std')
    ax[1][1].plot(10**y_test0,np.ones(len(y_test0)),color="red",linestyle="--")
    ax[1][1].plot(10**y_test0,np.zeros(len(y_test0)),color="red")
    ax[1][1].plot(10**y_test0,-np.ones(len(y_test0)),color="red",linestyle="--")
    ax[1][1].set_ylabel(r"$\frac{L_{10}E_{JENERGY}-L_{10}E_{True}}{L_{10}E_{True}}$")
    ...
    im = ax[2][1].hist2d(10**y_test0,np.abs((10**X_test0['JENERGY_ENERGY']-10**y_test0)/10**y_test0), bins = [x_bins,
                                                                                                          y_bins], range
                         =[xlim,[0,error_ylim[1]]], weights = X_test0['weights'], cmin = cmin)
    avg = avg_and_std(im[0],im[1],im[2], err=1)
    avg_x, avg_y, std = avg[:, 0], avg[:, 1], avg[:, 2]
    ax[2][1].errorbar(avg_x, avg_y, yerr=std, fmt='.', markersize=6, linewidth=1, c="black", label='weighted mean and std')
    ax[2][1].plot(10**y_test0,np.ones(len(y_test0)),color="red",linestyle="--")
    ax[2][1].plot(10**y_test0,0.5*np.ones(len(y_test0)),color="red",linestyle="--")
    ax[2][1].plot(10**y_test0,0.25*np.ones(len(y_test0)),color="red",linestyle="--")
    ...
    im = ax[3][1].hist2d(10**y_test0,np.abs((X_test0['JENERGY_ENERGY']-y_test0)/y_test0), bins = [x_bins,
                                                                                                            y_bins],
                         range =[xlim,[0,2.5]], weights = X_test0['weights'], cmin = cmin)
    avg = avg_and_std(im[0],im[1],im[2], err=1)
    avg_x, avg_y, std = avg[:, 0], avg[:, 1], avg[:, 2]
    ax[3][1].errorbar(avg_x, avg_y, yerr=std, fmt='.', markersize=6, linewidth=1, c="black", label='weighted mean and std')
    ax[3][1].plot(10**y_test0,np.ones(len(y_test0)),color="red",linestyle="--")
    ax[3][1].plot(10**y_test0,0.5*np.ones(len(y_test0)),color="red",linestyle="--")
    ax[3][1].plot(10**y_test0,0.25*np.ones(len(y_test0)),color="red",linestyle="--")
    ax[3][1].set_ylabel(r"$\frac{|L_{10}E_{JENERGY}-L_{10}E_{True}|}{L_{10}E_{True}}$")
    ...
    im = ax[0][2].hist2d(10**y_test0,((10**(X_test0['JSTART_LENGTH_METRES'])*0.25-10**y_test0)/10**y_test0), bins = [x_bins,
                                                                                                               y_bins],
                         range =[xlim,error_ylim], weights = X_test0['weights'], cmin = cmin)
    avg = avg_and_std(im[0],im[1],im[2], err=1)
    avg_x, avg_y, std = avg[:, 0], avg[:, 1], avg[:, 2]
    ax[0][2].errorbar(avg_x, avg_y, yerr=std, fmt='.', markersize=6, linewidth=1, c="black", label='weighted mean and std')
    ax[0][2].plot(10**y_test0,np.ones(len(y_test0)),color="red",linestyle="--")
    ...
    im = ax[1][2].hist2d(10**y_test0,((np.log10(10**(X_test0['JSTART_LENGTH_METRES'])*0.25)-y_test0)/y_test0),  bins = [x_bins, y_bins], range =[xlim,[-2.5,2.5]], weights = X_test0['weights'], cmin = cmin)
    avg = avg_and_std(im[0],im[1],im[2], err=1)
    avg_x, avg_y, std = avg[:, 0], avg[:, 1], avg[:, 2]
    ax[1][2].errorbar(avg_x, avg_y, yerr=std, fmt='.', markersize=6, linewidth=1, c="black", label='weighted mean and std')
    ax[1][2].plot(10**y_test0,np.ones(len(y_test0)),color="red",linestyle="--")
    ax[1][2].plot(10**y_test0,np.zeros(len(y_test0)),color="red")
    ax[1][2].plot(10**y_test0,-np.ones(len(y_test0)),color="red",linestyle="--")
    ax[1][2].set_ylabel(r"$\frac{L_{10}0.25*E_{JSTART-LENGTH}-L_{10}E_{True}}{L_{10}E_{True}}$")
    ...
    im = ax[2][2].hist2d(10**y_test0,np.abs((10**(X_test0['JSTART_LENGTH_METRES'])*0.25-10**y_test0)/10**y_test0), 
                         bins = [x_bins, y_bins], range =[xlim,[0,error_ylim[1]]], weights = X_test0['weights'], cmin = cmin)
    avg = avg_and_std(im[0],im[1],im[2], err=1)
    avg_x, avg_y, std = avg[:, 0], avg[:, 1], avg[:, 2]
    ax[2][2].errorbar(avg_x, avg_y, yerr=std, fmt='.', markersize=6, linewidth=1, c="black", label='weighted mean and std')
    ax[2][2].plot(10**y_test0,np.ones(len(y_test0)),color="red",linestyle="--")
    ax[2][2].plot(10**y_test0,0.5*np.ones(len(y_test0)),color="red",linestyle="--")
    ax[2][2].plot(10**y_test0,0.25*np.ones(len(y_test0)),color="red",linestyle="--")
    ax[2][2].set_ylabel(r"$\frac{|0.25*E_{JSTART-LENGTH}-E_{True}|}{E_{True}}$")
    ...
    im = ax[3][2].hist2d(10**y_test0,(np.abs(np.log10(10**(X_test0['JSTART_LENGTH_METRES'])*0.25)-y_test0)/y_test0), bins = [x_bins, y_bins], range =[xlim,[0,2.5]], weights = X_test0['weights'], cmin = cmin)
    avg = avg_and_std(im[0],im[1],im[2], err=1)
    avg_x, avg_y, std = avg[:, 0], avg[:, 1], avg[:, 2]
    ax[3][2].errorbar(avg_x, avg_y, yerr=std, fmt='.', markersize=6, linewidth=1, c="black", label='weighted mean and std')
    ax[3][2].plot(10**y_test0,np.ones(len(y_test0)),color="red",linestyle="--")
    ax[3][2].plot(10**y_test0,0.5*np.ones(len(y_test0)),color="red",linestyle="--")
    ax[3][2].plot(10**y_test0,0.25*np.ones(len(y_test0)),color="red",linestyle="--")
    ax[3][2].set_ylabel(r"$\frac{|L_{10}E_{JSTART-LENGTH}-L_{10}E_{True}|}{L_{10}E_{True}}$")
    ...
    if len(Gcols)>0:
        ...
        im = ax[0][3].hist2d(10**y_test0,((10**X_test0['pred_energy']-10**y_test0)/10**y_test0), 
                             bins = [x_bins, y_bins], range =[xlim,error_ylim], weights = X_test0['weights'], cmin = cmin)
        avg = avg_and_std(im[0],im[1],im[2], err=1)
        avg_x, avg_y, std = avg[:, 0], avg[:, 1], avg[:, 2]
        ax[0][3].errorbar(avg_x, avg_y, yerr=std, fmt='.', markersize=6, linewidth=1, c="black", label='weighted mean and std')
        ax[0][3].plot(10**y_test0,np.ones(len(y_test0)),color="red",linestyle="--")
        ...
        im = ax[1][3].hist2d(10**y_test0,((np.log10(10**X_test0['pred_energy'])-y_test0)/y_test0), 
                             bins = [x_bins, y_bins], range =[xlim,[-2.5,2.5]], weights = X_test0['weights'], cmin = cmin)
        avg = avg_and_std(im[0],im[1],im[2], err=1)
        avg_x, avg_y, std = avg[:, 0], avg[:, 1], avg[:, 2]
        ax[1][3].errorbar(avg_x, avg_y, yerr=std, fmt='.', markersize=6, linewidth=1, c="black", label='weighted mean and std')
        ax[1][3].plot(10**y_test0,np.ones(len(y_test0)),color="red",linestyle="--")
        ax[1][3].plot(10**y_test0,np.zeros(len(y_test0)),color="red")
        ax[1][3].plot(10**y_test0,-np.ones(len(y_test0)),color="red",linestyle="--")
        ax[1][3].set_ylabel(r"$\frac{L_{10}E_{GNN}-L_{10}E_{True}}{L_{10}E_{True}}$")
        ...
        im = ax[2][3].hist2d(10**y_test0,np.abs((10**X_test0['pred_energy']-10**y_test0)/10**y_test0), 
                             bins = [x_bins, y_bins],  range =[xlim,[0,error_ylim[1]]], weights = X_test0['weights'], cmin = cmin)
        avg = avg_and_std(im[0],im[1],im[2], err=1)
        avg_x, avg_y, std = avg[:, 0], avg[:, 1], avg[:, 2]
        ax[2][3].errorbar(avg_x, avg_y, yerr=std, fmt='.', markersize=6, linewidth=1, c="black", label='weighted mean and std')
        ax[2][3].plot(10**y_test0,np.ones(len(y_test0)),color="red",linestyle="--")
        ax[2][3].plot(10**y_test0,0.5*np.ones(len(y_test0)),color="red",linestyle="--")
        ax[2][3].plot(10**y_test0,0.25*np.ones(len(y_test0)),color="red",linestyle="--")
        ...
        im = ax[3][3].hist2d(10**y_test0,np.abs((np.log10(10**X_test0['pred_energy'])-y_test0)/y_test0), 
                             bins = [x_bins, y_bins], range =[xlim,[0,2.5]], weights = X_test0['weights'], cmin = cmin)
        avg = avg_and_std(im[0],im[1],im[2], err=1)
        avg_x, avg_y, std = avg[:, 0], avg[:, 1], avg[:, 2]
        ax[3][3].errorbar(avg_x, avg_y, yerr=std, fmt='.', markersize=6, linewidth=1, c="black", label='weighted mean and std')
        ax[3][3].plot(10**y_test0,np.ones(len(y_test0)),color="red",linestyle="--")
        ax[3][3].plot(10**y_test0,0.5*np.ones(len(y_test0)),color="red",linestyle="--")
        ax[3][3].plot(10**y_test0,0.25*np.ones(len(y_test0)),color="red",linestyle="--")
        ax[3][3].set_ylabel(r"$\frac{|L_{10}E_{GNN}-L_{10}E_{True}|}{L_{10}E_{True}}$")
