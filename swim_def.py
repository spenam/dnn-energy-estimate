...
class Add_SWIMinfo_neutrino(kp.Module):
    ...
    def process(self, blob):
        ...
        blob["tree"]["run_duration"] = frame["T.sum_mc_evt.livetime_DAQ"].astype(
            "float64"
        )
        ...
        blob["tree"]["pos_x_true"] = frame["E.mc_trks.pos.x[:,0]"].astype(
            "float64")
        blob["tree"]["pos_y_true"] = frame["E.mc_trks.pos.y[:,0]"].astype(
            "float64")
        blob["tree"]["pos_z_true"] = frame["E.mc_trks.pos.z[:,0]"].astype(
            "float64")
        blob["tree"]["pos_x_recoJGandalf"] = frame["E.trks.pos.x[:,0]"].astype(
            "float64"
        )
        blob["tree"]["pos_y_recoJGandalf"] = frame["E.trks.pos.y[:,0]"].astype(
            "float64"
        )
        blob["tree"]["pos_z_recoJGandalf"] = frame["E.trks.pos.z[:,0]"].astype(
            "float64"
        )
        ...
        blob["tree"]["energy_true"] = frame["E.mc_trks.E[:,0]"].astype(
            "float64")
        blob["tree"]["cos_zenith_true"] = np.float64(-1) * frame[
            "E.mc_trks.dir.z[:,0]"
        ].astype("float64")
        blob["tree"]["bjorken_y_true"] = frame["T.sum_mc_nu.by"].astype(
            "float64")
        blob["tree"]["energy_recoJEnergy"] = frame["E.trks.E[:,0]"].astype(
            "float64"
        )  # energy_recoTracklength
        blob["tree"]["energy_recoTracklength"] = np.float64(0.25) * frame[
            "E.trks.len[:,0]"
        ].astype(
            "float64"
        )  # cos_zenith_recoJGandalf
        blob["tree"]["energy_recoRatioEL_JEnergy"] = frame["E.trks.E[:,0]"].astype(
            "float64"
        ) / (np.float64(2.0 * 6371.0088) * frame["E.trks.len[:,0]"].astype("float64"))
        blob["tree"]["energy_recoRatioLE_JEnergy"] = (
            np.float64(2.0 * 6371.0088) *
            frame["E.trks.len[:,0]"].astype("float64")
        ) / frame["E.trks.E[:,0]"].astype("float64")
        blob["tree"]["energy_recoRatioEL_Tracklen"] = (
            np.float64(0.25) * frame["E.trks.len[:,0]"].astype("float64")
        ) / (np.float64(2.0 * 6371.0088) * frame["E.trks.len[:,0]"].astype("float64"))
        blob["tree"]["energy_recoRatioLE_Tracklen"] = (
            np.float64(2.0 * 6371.0088) *
            frame["E.trks.len[:,0]"].astype("float64")
        ) / (np.float64(0.25) * frame["E.trks.len[:,0]"].astype("float64"))
        blob["tree"]["cos_zenith_recoJGandalf"] = np.float64(-1) * frame[
            "E.trks.dir.z[:,0]"
        ].astype("float64")
        ...
        blob["tree"]["maximumToT_triggerHit"] = frame[
            "T.feat_Neutrino2020.maximumToT_triggerHit"
        ]
        ...
        blob["tree"]["w2"] = frame["E.w[:,1]"] * \
            np.float64(365.25 * 24.0 * 60.0 * 60.0)
        blob["tree"]["int_len"] = frame["E.w2list[:,5]"]
        blob["tree"]["ngen"] = frame["T.sum_mc_evt.n_gen"].astype("float64")
        blob["tree"]["EG"] = frame["E.w2list[:,1]"]
        blob["tree"]["E_min_gen"] = frame["T.sum_mc_evt.E_min_gen"].astype(
            "float64")
        blob["tree"]["E_max_gen"] = frame["T.sum_mc_evt.E_max_gen"].astype(
            "float64")
...
class Add_SWIMinfo_muon(kp.Module):
    def configure(self):
        ...
        # Register the service to append branches to the pump
        self.require_service("branches")
        self.totaltime = self.get(
            "totaltime", default=460.7983564814815 * 60 * 60 * 24)
    ...
    def process(self, blob):
        ...
        blob["tree"]["bjorken_y_true"] = frame["bjorken_y_true"].astype(
            "float64")
        blob["tree"]["energy_recoJEnergy"] = frame["E.trks.E[:,0]"].astype(
            "float64"
        )  # energy_recoTracklength
        ...
        frame["cos_zenith_recoDusj"] = np.float64(-0.5)
        frame["bjorken_y_recoDusj"] = np.float64(0.5)
        blob["tree"]["energy_recoDusj"] = frame["energy_recoDusj"].astype(
            "float64")
        blob["tree"]["cos_zenith_recoDusj"] = frame["cos_zenith_recoDusj"].astype(
            "float64"
        )
        blob["tree"]["bjorken_y_recoDusj"] = frame["bjorken_y_recoDusj"].astype(
            "float64"
        )
        ...
        frame["weight_one_year"] = (
            frame["T.sum_mc_evt.livetime_DAQ"]
            * np.float64(3.15576e7)
            / (frame["T.sum_mc_evt.livetime_sim"] * np.float64(self.totaltime))
        )
        blob["tree"]["weight_one_year"] = frame["weight_one_year"].astype(
            "float64")
        blob["tree"]["antimu_proba_bkg"] = frame["tree.muonscore"]
        blob["tree"]["pid_proba_track"] = frame["tree.trackscore"]
        blob["tree"]["rectype_JGandalf"] = frame["E.trks.rec_type[:,0]"]
...
class Add_SWIMinfo_JSHF(kp.Module):
    ...
    def process(self, blob):
        ...
        blob["tree"]["pos_x_recoJShower"] = frame["E.trks.pos.x[:,1]"].astype(
            "float64")
        blob["tree"]["pos_y_recoJShower"] = frame["E.trks.pos.y[:,1]"].astype(
            "float64")
        blob["tree"]["pos_z_recoJShower"] = frame["E.trks.pos.z[:,1]"].astype(
            "float64")

        blob["tree"]["energy_recoJShower"] = frame["E.trks.E[:,1]"].astype(
            "float64")
        blob["tree"]["energy_recoRatioEL_JShower"] = frame["E.trks.E[:,1]"].astype(
            "float64"
        ) / (np.float64(2.0 * 6371.0088) * frame["E.trks.len[:,0]"].astype("float64"))
        blob["tree"]["energy_recoRatioLE_JShower"] = (
            np.float64(2.0 * 6371.0088) *
            frame["E.trks.len[:,0]"].astype("float64")
        ) / frame["E.trks.E[:,1]"].astype("float64")
        blob["tree"]["cos_zenith_recoJShower"] = np.float64(-1) * frame[
            "E.trks.dir.z[:,1]"
        ].astype("float64")
        frame["bjorken_y_recoJShower"] = np.float64(0.5)
        blob["tree"]["bjorken_y_recoJShower"] = frame["bjorken_y_recoJShower"].astype(
            "float64"
        )
...
class Add_SWIMinfo_DNN(kp.Module):
    ...
    def process(self, blob):
        ...
        frame = pd.DataFrame(blob["tree"])
        # print(frame.keys())
        blob["tree"]["energy_recoDNN"] = frame["DNN.energy_recoDNN"].astype(
            "float64")
        blob["tree"]["energy_recoRatioEL_DNN"] = frame["DNN.energy_recoDNN"].astype(
            "float64"
        ) / (np.float64(2.0 * 6371.0088) * frame["E.trks.len[:,0]"].astype("float64"))
        blob["tree"]["energy_recoRatioLE_DNN"] = (
            np.float64(2.0 * 6371.0088) *
            frame["E.trks.len[:,0]"].astype("float64")
        ) / frame["DNN.energy_recoDNN"].astype("float64")
