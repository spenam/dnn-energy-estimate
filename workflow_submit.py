...
JGandalfcols = ['JENERGY_ENERGY','JSTART_LENGTH_METRES','dir_x_gandalf', 'dir_y_gandalf', 'dir_z_gandalf'] # Features to keep
JShowerfitcols = ['JSHOWERFIT_LENGTH_METRES','JSHOWERFIT_ENERGY', 'dir_x_showerfit', 'dir_y_showerfit', 'dir_z_showerfit']
...
lkl = ["0", "5"] # 0 for training without likelihood and 1 for training with likelihood
...
def do_samples():
    ...
    script_options = ""
    time = "00:30:00"
    print('Submitting : sbatch --partition htc --nodes 1 --job-name make_training_samples --ntasks 1 --mem-per-cpu 64GB --time '+time+ ' ' + batch_job_script + ' ' + script_options)
    subprocess.run(['sbatch --partition htc --nodes 1 --job-name make_training_samples --ntasks 1 --mem-per-cpu 64GB --time '+time+ ' ' + batch_job_script + ' ' + script_options],shell=True)
                ...
                for i in range(len(architecture_size)):
                    for j in range(len(lr)):
                        ...
                        if online==1:
                            ...
                            subprocess.run([batch_job_script + ' ' + script_options],shell=True)
