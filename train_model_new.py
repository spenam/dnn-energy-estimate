...
def plot_loss(history):
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig("plots/" + NN_info + "/w/" + str(LR) +
                "/loss_" + n_features + ".pdf")
    plt.savefig("plots/" + NN_info + "/w/" + str(LR) +
                "/loss_" + n_features + ".png")
    plt.grid(True)
    # plt.show()
    plt.clf()
...
for i in [lkl]:
    ...
    with h5py.File("for_train.h5", "r") as f:
        with h5py.File("for_val.h5", "r") as g:
            ...
            if lkl == 0:
                X_train_scaled = tf.convert_to_tensor(
                    pd.DataFrame(
                        f["X"][
                            "JSHOWERFIT_LENGTH_METRES",
                            "JSHOWERFIT_ENERGY",
                ...
                X_val_scaled = tf.convert_to_tensor(
                    pd.DataFrame(
                        g["X"][
                            "JSHOWERFIT_LENGTH_METRES",
                            "JSHOWERFIT_ENERGY",
        ...
        model.save("models_w/" + NN_info + "_w_" +
                   n_features + "_" + str(LR) + ".h5")
