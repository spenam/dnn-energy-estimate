...
PRED = JGANDALFPRED[(JGANDALFPRED["run_id"] < 10000) &
                    (JGANDALFPRED["run_id"] > 9000)]
...
for i in [0]:
    ...
    if len(Gcols) != 0:
        ...
        nan_mask = (
            (data["JENERGY_ENERGY"] != -np.inf)
            & (data["JSHOWERFIT_ENERGY"] != -np.inf)
            & (data["pred_energy"] != -np.inf)
            & (data["JSTART_LENGTH_METRES"] != -np.inf)
        )
    else:
        nan_mask = (
            (data["JENERGY_ENERGY"] != -np.inf)
    ...
    with h5py.File("shuffled_dsts.h5", "w") as f:
        f["data"] = data_shuffled.drop(
            support_features, 1).to_records(index=False)
    ...
    data_train, data_test = train_test_split(
        data, test_size=0.3, random_state=3)
    Ebins = np.logspace(0, 2, nbines)
    plt.hist(data_train["mc_energy_dst"], bins=Ebins,
    ...
    cdata = bootstrap_sampling(
        data_train, uplim=uplim, lowE=0, highE=1.3, nbins=int(nbines / 2)
    )
    cdatahigh = bootstrap_sampling(
        data_train, uplim=uplim / 3, lowE=1.3, highE=2, nbins=int(nbines / 2)
    )
    ...
    X_all = cdata.drop(
        ["mc_energy_dst", "mc_pos_x", "mc_pos_y", "mc_pos_z"], 1)
    if len(Gcols) != 0:
        # nan_mask = (X_all['JENERGY_ENERGY'] != -np.inf) & (X_all['pred_energy'] != -np.inf) &        (X_all['JSTART_LENGTH_METRES'] != -np.inf)
        nan_mask = (
            (X_all["JENERGY_ENERGY"] != -np.inf)
            & (X_all["JSHOWERFIT_ENERGY"] != -np.inf)
            & (X_all["pred_energy"] != -np.inf)
            & (X_all["JSTART_LENGTH_METRES"] != -np.inf)
        )
